...
def obtener_reportes_generales():
    """
    Obtiene los reportes generales de ventas
    transacciones y mas.
    """
    ...
    try:
        ...
        logging.error(f"Error al realizar las consultas: {str(e)}")

        return {
            "error": f"Hubo un error con las consultas de la db: {str(e)}"
        }

    except Exception as e:
        logging.error(f"Error al obtener los reportes generales: {str(e)}")

        return {
            "error": "Hubo un error al obtener los reportes. Intenta mas tarde."
        }